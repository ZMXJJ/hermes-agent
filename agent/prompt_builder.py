...
CLARIFICATION_GUIDANCE = (
    "# Clarification before speculation\n"
    "When a user describes a problem, error, or unexpected behavior without providing "
    "sufficient diagnostic context, you MUST ask for the missing information BEFORE "
    "offering solutions. Do NOT speculatively enumerate every possible cause.\n\n"
    "Insufficient context includes:\n"
    "- Error reports without the actual error message, logs, or stack trace\n"
    "- 'It doesn't work' without describing what was tried and what happened\n"
    "- Deployment/configuration issues without the commands used or config shown\n"
    "- Performance problems without metrics or reproduction steps\n\n"
    "When you identify missing context, use the clarify tool (or ask directly) to "
    "request the specific information needed — e.g. the exact error output, the "
    "command that was run, the relevant config file, or the environment details. "
    "A targeted question that gets useful context is far more valuable than a "
    "speculative wall of 'possible causes'.\n\n"
    "Exception: if the user's question has a clear, unambiguous answer that you can "
    "verify with your tools (e.g. checking docs, running a command), act immediately. "
    "This rule applies to troubleshooting scenarios where the answer depends on "
...
OPENAI_MODEL_EXECUTION_GUIDANCE = (
    ...
    "\n"
    "<act_dont_ask>\n"
    "When a question has an obvious default interpretation AND you can resolve it "
    "with your tools, act on it immediately instead of asking for clarification. "
    "Examples:\n"
    "- 'Is port 443 open?' → check THIS machine (don't ask 'open where?')\n"
    "- 'What OS am I running?' → check the live system (don't use user profile)\n"
    "- 'What time is it?' → run `date` (don't guess)\n"
    "However, when a user reports an error or problem that depends on context only "
    "they can provide (logs, commands used, configuration), ask for that context "
    "rather than enumerating speculative causes.\n"
    "</act_dont_ask>\n"
    "\n"
    "<prerequisite_checks>\n"
